...
namespace cs_packages.test
{
    class handling : Events.Script
    {
        ...
        public handling()
        {
            ...
            Events.Add("add_SetHandling", SetHandling);
        }


        public void SetHandling(object[] args)
        {
            Chat.Output("1233213");

            ushort vehid = Convert.ToUInt16(args[0]);
            ushort remoteId = Convert.ToUInt16(args[1]);


            Vehicle veh = new Vehicle(vehid, remoteId);//Надо тестить, скорее всего неправильные ID

            

            Chat.Output($"{vehid}");
            Chat.Output($"{remoteId}");
